...
/// Example using FastIOAdvanced and Test class
...
public class Example {
    ...
    public static void enki() {
        FastIOAdvanced f = new FastIOAdvanced();
        int n = f.nextInt();
        int q = f.nextInt();
        List<Integer> h = f.nextIntList();
        ...
        ArrayList<Integer> stack = new ArrayList<>(Arrays.asList(q));
        visited.set(q, true);
        ...
        while (!stack.isEmpty()) {
            ...
            if (j < n - 1 && !visited.get(j + 1) && h.get(j + 1) >= h.get(j)) {
                visited.set(j + 1, true);
                stack.add(j + 1);
            }
        }

        System.out.println(visited.stream().mapToInt((i) -> i ? 1 : 0).sum());

    }

    /**
     * Ultra-optimized FastIO for USACO competitions Features: Async I/O,
     * Variable-size arrays, Mathematical utilities, Memory optimization, Thread
     * safety, Error recovery
     */
    public static class FastIOAdvanced implements AutoCloseable {
        ...
        public FastIOAdvanced() {
            this(System.in, System.out);
        }

        public FastIOAdvanced(InputStream in, OutputStream out) {
        ...
        public FastIOAdvanced(String inputFile) throws IOException {
            this(new FileInputStream(inputFile), System.out);
        }

        public FastIOAdvanced(String inputFile, String outputFile) throws IOException {
        ...
        /**
         * Reads all remaining integers from current line as an array Perfect
         * for when array size is not predetermined
         */
        ...
        /**
         * Reads a 2D matrix until end of input (no dimensions required) Perfect
         * for USACO where matrix is always the last parameter
         */
        ...
        /**
         * Reads a character matrix until end of input Each line becomes a row
         * of characters
         */
